...
// include the byte array "twlbg_patch_code" containing the TwlBg patch
#ifdef INCLUDE_NEW_3DS_STUFF
    #include "../arm11_twlbg_patch/twl_bg_patch_bytes.with_nub.h"
#else
    #include "../arm11_twlbg_patch/twl_bg_patch_bytes.no_nub.h"
#endif

static inline void flush_cache(u8 *addr, u32 size) {
    // this is a system call called "AddCodeSegment" (according to 3dbrew)
    // it flushes the data cache and invalidates the instruction cache
    // (which is necessary for self-modifying code)
