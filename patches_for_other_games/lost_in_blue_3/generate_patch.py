...
RomInfo = namedtuple(
    'RomInfo',
    'arm7_ipc_send_msg, arm7_set_ipc_channel_handler, arm7_rtc_region_start, arm7_vblank_irq_end_address, arm7_rtc_init_function_call, arm9_code_position, arm9_hook_walk, arm9_hook_run, arm9_get_angle_func, description')
roms_info = {
    "YL3P-F064CA6C": RomInfo(0x037FE2C4, 0x037FE250, 0x027F5A94, 0x037F84D8, 0x037F8414, 0x02000A78, 0x02077C08, 0x02077BCC, 0x02002954, "Europe v1.0"),
    "YL3J-C3FD688F": RomInfo(0x037FE2C4, 0x037FE250, 0x027F5A94, 0x037F84D8, 0x037F8414, 0x02000A78, 0x02076F2C, 0x02076EF0, 0x02002954, "Japan v1.0"),
    "YL3J-70924DF7": RomInfo(0x037FE2C4, 0x037FE250, 0x027F5A94, 0x037F84D8, 0x037F8414, 0x02000A78, 0x020772E0, 0x020772A4, 0x02002954, "Japan v1.1"),
    "YL3E-35590DB5": RomInfo(0x037FE2C4, 0x037FE250, 0x027F5A94, 0x037F84D8, 0x037F8414, 0x02000A78, 0x0207783C, 0x02077800, 0x02002954, "USA v1.0"),
}
...
def assemble_arm9_patch(rom_signature, asm_filename):
    ...
    arm9_defines_for_assembler = [
        ("GET_ANGLE_FUNC", rom_info.arm9_get_angle_func),
    ]
...
def generate_action_replay_code(rom_signature):
    ...
    # Movement (Walking)
    walk_hook_addr = rom_info.arm9_hook_walk
    orig_instr = 0xe1a01821
    offset = find_function_offset_in_asm_listing(code_asm_text, "move_with_cpad")
    walk_branch_instr = instr__arm_b(walk_hook_addr, arm9_start_address + offset, True)

    # Movement (Running, i.e. when B is pressed)
    run_hook_addr = rom_info.arm9_hook_run
    run_branch_instr = instr__arm_b(run_hook_addr, arm9_start_address + offset, True)
    ...
    ar_code += f"""
        ...
        # Walking
        427FFC3C 00000150
            5{walk_hook_addr:07X} {orig_instr:08X}
                0{walk_hook_addr:07X} {walk_branch_instr:08X}
        D2000000 00000000

        # Running
        427FFC3C 00000150
            5{run_hook_addr:07X} {orig_instr:08X}
                0{run_hook_addr:07X} {run_branch_instr:08X}
        D2000000 00000000
