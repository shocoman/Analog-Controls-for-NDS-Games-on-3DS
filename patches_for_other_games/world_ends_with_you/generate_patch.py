...
RomInfo = namedtuple(
    'RomInfo',
    'arm7_ipc_send_msg, arm7_set_ipc_channel_handler, arm7_rtc_region_start, arm7_vblank_irq_end_address, arm7_rtc_init_function_call, arm9_code_position, arm9_getangle_func, arm9_vec_normalize_func, movement_hook, is_moving_a_hook, is_moving_b_hook, combat_movement_hook, combat_is_moving_a_hook, combat_is_moving_b_hook, description')
roms_info = {
    "AWLP-C5889C49": RomInfo(0x037FE3DC, 0x037FE368, 0x027F5CA0, 0x037F84D8, 0x037F8414, 0x02000BDC, 0x02035094, 0x02034EA4, 0x020B288C, 0x020AEED8, 0x020B2878, 0x020906C8, 0x0208F8DC, 0x02090868, "Europe v1.0"),
    "AWLJ-34608EC0": RomInfo(0x037FE29C, 0x037FE228, 0x027F5B18, 0x037F84D8, 0x037F8414, 0x02000BA4, 0x02034D2C, 0x02034B3C, 0x020B2F90, 0x020AF5D4, 0x020B2F7C, 0x02090F34, 0x02090148, 0x020910D4, "Japan v1.1"),
    "AWLE-038D607C": RomInfo(0x037FE3DC, 0x037FE368, 0x027F5CA0, 0x037F84D8, 0x037F8414, 0x02000BDC, 0x02035094, 0x02034EA4, 0x020B288C, 0x020AEED8, 0x020B2878, 0x020906C8, 0x0208F8DC, 0x02090868, "USA v1.0"),
}
...
def assemble_arm9_patch(rom_signature, asm_filename):
    ...
    arm9_defines_for_assembler = [
        ...
        ("VEC_NORMALIZE_FUNC", rom_info.arm9_vec_normalize_func),
    ]
...
def generate_action_replay_code(rom_signature, include_cstick_support):
    ...
    ####################################################################################
    # Arm9 patch
    arm9_start_address = rom_info.arm9_code_position # 0x02000bdc
    code_binary, code_asm_text = assemble_arm9_patch(rom_signature, "arm9_controls_hook.s")

    # Movement
    movement_hook_addr = rom_info.movement_hook # 0x020b288c
    movement_orig_instr = 0xe1d71ab8
    movement_offset = find_function_offset_in_asm_listing(code_asm_text, "move_with_cpad")
    movement_branch_instr = instr__arm_b(movement_hook_addr, arm9_start_address + movement_offset, True)

    # is moving a
    is_moving_a_hook_addr = rom_info.is_moving_a_hook # 0x020aeed8
    is_moving_a_orig_instr = 0xe1d020b0
    is_moving_a_offset = find_function_offset_in_asm_listing(code_asm_text, "is_moving_a")
    is_moving_a_branch_instr = instr__arm_b(is_moving_a_hook_addr, arm9_start_address + is_moving_a_offset, True)

    # is moving b
    is_moving_b_hook_addr = rom_info.is_moving_b_hook # 0x020b288c - 0x14
    is_moving_b_orig_instr = 0xe1d110b0 
    is_moving_b_offset = find_function_offset_in_asm_listing(code_asm_text, "is_moving_b")
    is_moving_b_branch_instr = instr__arm_b(is_moving_b_hook_addr, arm9_start_address + is_moving_b_offset, True)

    # COMBAT: is moving a
    combat_is_moving_a_hook_addr = rom_info.combat_is_moving_a_hook # 0x0208f8dc
    combat_is_moving_a_orig_instr = 0xe3a00006
    combat_is_moving_a_offset = find_function_offset_in_asm_listing(code_asm_text, "combat_is_moving_a")
    combat_is_moving_a_branch_instr = instr__arm_b(combat_is_moving_a_hook_addr, arm9_start_address + combat_is_moving_a_offset, True)
    
    # COMBAT: is moving b
    combat_is_moving_b_hook_addr = rom_info.combat_is_moving_b_hook # 0x02090868
    combat_is_moving_b_orig_instr = 0xe3a00006
    combat_is_moving_b_offset = find_function_offset_in_asm_listing(code_asm_text, "combat_is_moving_b")
    combat_is_moving_b_branch_instr = instr__arm_b(combat_is_moving_b_hook_addr, arm9_start_address + combat_is_moving_b_offset, True)
    
    
    # COMBAT: movement
    combat_movement_hook_addr = rom_info.combat_movement_hook # 0x020906c8
    combat_movement_orig_instr = 0xe3a00001
    combat_movement_offset = find_function_offset_in_asm_listing(code_asm_text, "combat_movement")
    combat_movement_branch_instr = instr__arm_b(combat_movement_hook_addr, arm9_start_address + combat_movement_offset, True)
    ...
    ar_code += f"""
        ...
        # ======================================================
        # general movement with CPad

        427FFC3C 00000150
            5{movement_hook_addr:07X} {movement_orig_instr:08X}
                0{movement_hook_addr:07X} {movement_branch_instr:08X}
        D2000000 00000000
        
        427FFC3C 00000150
            5{is_moving_a_hook_addr:07X} {is_moving_a_orig_instr:08X}
                0{is_moving_a_hook_addr:07X} {is_moving_a_branch_instr:08X}
        D2000000 00000000
        
        427FFC3C 00000150
            5{is_moving_b_hook_addr:07X} {is_moving_b_orig_instr:08X}
                0{is_moving_b_hook_addr:07X} {is_moving_b_branch_instr:08X}
        D2000000 00000000
        ...
        427FFC3C 00000150
            5{combat_is_moving_a_hook_addr:07X} {combat_is_moving_a_orig_instr:08X}
                0{combat_is_moving_a_hook_addr:07X} {combat_is_moving_a_branch_instr:08X}
        D2000000 00000000

        427FFC3C 00000150
            5{combat_is_moving_b_hook_addr:07X} {combat_is_moving_b_orig_instr:08X}
                0{combat_is_moving_b_hook_addr:07X} {combat_is_moving_b_branch_instr:08X}
        D2000000 00000000

        427FFC3C 00000150
            5{combat_movement_hook_addr:07X} {combat_movement_orig_instr:08X}
                0{combat_movement_hook_addr:07X} {combat_movement_branch_instr:08X}
        D2000000 00000000
