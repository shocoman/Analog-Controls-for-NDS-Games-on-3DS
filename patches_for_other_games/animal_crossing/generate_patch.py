...
ARM9_CODE_START_ADDRESS = 0x02000a5c # there goes the arm9 memory protection unit setup
...
RomInfo = namedtuple(
    'RomInfo',
    'arm7_ipc_send_msg, arm7_set_ipc_channel_handler, arm7_rtc_region_start, arm7_vblank_irq_end_address, arm7_rtc_init_function_call, arm9_controls_hook_insertion_point, arm9_sqrt64_func, arm9_div32_func, arm9_getangle_func, description')
roms_info = {
    "ADMP-53DBB069": RomInfo(0x037FB5F4, 0x037FB6C0, 0x03804DD0, 0x037F86F4, 0x037F84E8, 0x0200DDEA, 0x020EA638, 0x02136534, 0x020E8818, "Europe v1.0"),
    "ADMJ-99A83574": RomInfo(0x037FB5F4, 0x037FB6C0, 0x03804DD0, 0x037F86F4, 0x037F84E8, 0x0200DD16, 0x020E9818, 0x021326F0, 0x020E79F8, "Japan v1.0"),
    "ADMJ-42DE9026": RomInfo(0x037FB5F4, 0x037FB6C0, 0x03804DD0, 0x037F86F4, 0x037F84E8, 0x0200DD0E, 0x020E9BB8, 0x02132BF0, 0x020E7D98, "Japan v1.1"),
    "ADME-8F7851CB": RomInfo(0x037FB5F4, 0x037FB6C0, 0x03804DD0, 0x037F86F4, 0x037F84E8, 0x0200DD1A, 0x020E95D8, 0x02132BD0, 0x020E77B8, "USA v1.0"),
    "ADME-6C56B746": RomInfo(0x037FB5F4, 0x037FB6C0, 0x03804DD0, 0x037F86F4, 0x037F84E8, 0x0200DD16, 0x020E99B8, 0x02133150, 0x020E7B98, "USA v1.1"),
}
...
def assemble_arm9_patch(rom_signature, asm_filename):
    addresses = roms_info[rom_signature]

    arm9_defines_for_assembler = [("SQRT64_FUNC", addresses.arm9_sqrt64_func),
                                  ("DIV32_FUNC", addresses.arm9_div32_func),
                                  ("GETANGLE_FUNC", addresses.arm9_getangle_func)]
    ...
    return bytearray(open(f"{tmp_folder_name}/{patch_name}.bin", 'rb').read())
...
def generate_action_replay_code(rom_signature):
    ...
    arm7_code_start_address = addresses.arm7_rtc_region_start
    vblank_handler_end = addresses.arm7_vblank_irq_end_address
    ...
    ar_code += f"""
        ...
        5{vblank_handler_end:07X} {0xe12fff1e:08X}  # if equal to "bx lr"
            0{addresses.arm7_rtc_init_function_call:07X} 00000000               # prevent call to init the RTC on Arm7
    ...
    ####################################################################################
    # Arm9 Controls Patch
    start_address = ARM9_CODE_START_ADDRESS
    branch_from_addr = addresses.arm9_controls_hook_insertion_point
    orig_instruction = 0x9801
    code_binary = assemble_arm9_patch(rom_signature, "arm9_controls_hook.s")
    new_branch_instr = instr__thumb_bl(branch_from_addr, start_address, True)

    ar_code += f"""
        9{branch_from_addr:07X} {orig_instruction:08X}
            {ar_code__bulk_write(code_binary, start_address)}
            1{branch_from_addr:07X} {new_branch_instr[0]:08X}
            1{branch_from_addr+2:07X} {new_branch_instr[1]:08X}
        D0000000 00000000
    """
