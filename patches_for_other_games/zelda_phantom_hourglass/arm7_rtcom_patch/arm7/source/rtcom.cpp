...
__attribute__((section(".text"))) static u16 OLD_REG_KEYINPUT = 0;
__attribute__((section(".text"))) static u16 WAS_LID_CLOSED_LAST_FRAME = 0;
...
__attribute__((target("arm"))) void Update_RTCom() {
    ...
    enum RtcomStateTime {
        ...
        ToggleDPadEmul = RemoveSkipBranch + 50,
        ReadyToRead = ToggleDPadEmul + 10,
    };
    ...
    case ToggleDPadEmul: {
        ...
        if (*(u32 *)0x0205860C != 0xe1d660b0 && *(u32 *)0x02058668 != 0xe1d660b0) {
    ...
    case ReadyToRead: {
        ...
        // toggle the DPad emulation
        // only if L+R+Down are pressed
        u16 new_keyinput = (~REG_KEYINPUT) & 0x380;
        u16 pressed = new_keyinput & (new_keyinput ^ OLD_REG_KEYINPUT);
        if ((new_keyinput | pressed) == 0x380 && pressed != 0) {
            Execute_Code_Async_via_RTCom(6);
        }
        OLD_REG_KEYINPUT = new_keyinput;
