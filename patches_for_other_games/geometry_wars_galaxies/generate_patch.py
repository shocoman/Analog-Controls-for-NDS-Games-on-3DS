...
RomInfo = namedtuple(
    'RomInfo',
    'arm7_ipc_send_msg, arm7_set_ipc_channel_handler, arm7_rtc_region_start, arm7_vblank_irq_end_address, arm7_rtc_init_function_call, arm9_code_position, movement_hook, firing_hook, vector_normalize_func, description')
roms_info = {
    "YGLP-A0F2225A": RomInfo(0x037FF408, 0x037FF394, 0x027F5E20, 0x037FB784, 0x037F8438, 0x02000A78, 0x02013E18, 0x02013EB4, 0x0207AB78, "Europe v1.0"),
    "YGLE-142C9F7B": RomInfo(0x037FF408, 0x037FF394, 0x027F5E20, 0x037FB784, 0x037F8438, 0x02000A78, 0x02013E18, 0x02013EB4, 0x0207AB7C, "USA v1.0"),
}
...
def assemble_arm9_patch(rom_signature, asm_filename):
    ...
    arm9_defines_for_assembler = [
        ("VECTOR_NORMALIZE_FUNC", rom_info.vector_normalize_func),
    ]
...
def generate_action_replay_code(rom_signature, include_cstick_support):
    ...
    # Movement
    movement_hook_addr = rom_info.movement_hook
    movement_orig_instr = 0xe28d0014
    movement_offset = find_function_offset_in_asm_listing(code_asm_text, "movement")
    movement_branch_instr = instr__arm_b(movement_hook_addr, arm9_start_address + movement_offset, True)

    # Firing
    firing_hook_addr = rom_info.firing_hook
    firing_orig_instr = 0xe58dc0ec
    firing_offset = find_function_offset_in_asm_listing(code_asm_text, "firing")
    firing_branch_instr = instr__arm_b(firing_hook_addr, arm9_start_address + firing_offset, True)
    ...
    ar_code += f"""
        ...
        427FFC3C 00000150
            5{firing_hook_addr:07X} {firing_orig_instr:08X}
                0{firing_hook_addr:07X} {firing_branch_instr:08X}
        D2000000 00000000
...
def generate_action_replay_codes_for_all_rom_versions():
    ...
    for rom_id, info in roms_info.items():
        ...
        code_text = generate_action_replay_code(rom_id, True)
