...
RomInfo = namedtuple(
    'RomInfo',
    'arm7_ipc_send_msg, arm7_set_ipc_channel_handler, arm7_rtc_region_start, arm7_vblank_irq_end_address, arm7_rtc_init_function_call, arm9_dir_hook, arm9_speed_hook, arm9_code_position, arm9_dir_between_two_points_func, arm9_vector2d_length_func, description')
roms_info = {
    "BLHP-058619EE":
    RomInfo(
        0x037FEBE4, 0x037FEB74, 0x027E0000, 0x037F8464, 0x037F8324, 0x0204B3BC, 0x0204B418, 0x02000A78,
        0x020ECD30, 0x020BE52C, "Europe v1.0"),
    "BLHE-36AB8FCA":
    RomInfo(
        0x037FEBE4, 0x037FEB74, 0x027E0000, 0x037F8464, 0x037F8324, 0x0204B348, 0x0204B3A4, 0x02000A78,
        0x020ECCBC, 0x020BE4B8, "USA v1.0"), }
...
def assemble_arm9_patch(rom_signature, asm_filename):
    ...
    arm9_defines_for_assembler = [
        ("DIRECTION_BETWEEN_POINTS_FUNC", rom_info.arm9_dir_between_two_points_func),
        ("VECTOR2D_LENGTH_FUNC", rom_info.arm9_vector2d_length_func)
    ]
...
def generate_action_replay_code(rom_signature):
    ...
    # Set direction
    set_dir_hook_addr = rom_info.arm9_dir_hook
    set_dir_orig_instr = 0xe7d01009
    offset = find_function_offset_in_asm_listing(code_asm_text, "set_direction")
    set_dir_branch_instr = instr__arm_b(set_dir_hook_addr, arm9_start_address + offset, True)

    set_speed_hook_addr = rom_info.arm9_speed_hook
    offset = find_function_offset_in_asm_listing(code_asm_text, "set_speed")
    set_speed_branch_instr = instr__arm_b(set_speed_hook_addr, arm9_start_address + offset, True)

    ar_code += f"""
        # wait for some time, just to be sure (0x27FFC3C is a frame counter)
        42FFFC3C 00000200
            # check if we can hook the "player movement"
            5{set_dir_hook_addr:07X} {set_dir_orig_instr:08X}
                {ar_code__bulk_write(code_binary, arm9_start_address)} # main patch
                0{set_dir_hook_addr:07X} {set_dir_branch_instr:08X}
                0{set_speed_hook_addr:07X} {set_speed_branch_instr:08X}
        D2000000 00000000
    """
