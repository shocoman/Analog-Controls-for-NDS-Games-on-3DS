...
RomInfo = namedtuple(
    'RomInfo',
    'arm7_ipc_send_msg, arm7_set_ipc_channel_handler, arm7_rtc_region_start, arm7_vblank_irq_end_address, arm7_rtc_init_function_call, arm9_code_position, move_hook, speed_walk_hook, speed_jump_hook, get_angle_func, sqrt_func, sin_cos_lookup_table, description')
roms_info = {
    "APWP-CD5CACD1": RomInfo(0x037FE2F0, 0x037FCCD0, 0x027F5848, 0x037FB610, 0x037F84E4, 0x02000A14, 0x02022BC4, 0x020227CC, 0x0202267C, 0x02002C30, 0x020027EC, 0x02070764, "Europe v1.0"),
    "APWE-6124F30C": RomInfo(0x037FE2F0, 0x037FCCD0, 0x027F5848, 0x037FB610, 0x037F84E4, 0x02000A14, 0x02023114, 0x02022CF4, 0x02022BA0, 0x02002C90, 0x0200284C, 0x02071364, "USA v1.0"),
}
...
def assemble_arm9_patch(rom_signature, asm_filename):
    ...
    arm9_defines_for_assembler = [
        ...
        ("SINCOS_LOOKUP_TABLE", rom_info.sin_cos_lookup_table),
        ("SQRT_FUNC_ADDR", rom_info.sqrt_func),
    ]
...
def generate_action_replay_code(rom_signature):
    ...
    movement_orig_instr = 0xe3500000
    offset = find_function_offset_in_asm_listing(code_asm_text, "move_with_cpad")
    movement_branch_instr = instr__arm_b(movement_hook_addr, arm9_start_address + offset, True)

    speed_hook_addr = rom_info.speed_walk_hook
    offset = find_function_offset_in_asm_listing(code_asm_text, "control_speed_with_cpad__walking")
    speed_branch_instr = instr__arm_b(speed_hook_addr, arm9_start_address + offset, True, True)

    speed_jumping_hook_addr = rom_info.speed_jump_hook
    if rom_info.description == "Europe v1.0":
        speed_jumping_hook_addr += 4
    offset = find_function_offset_in_asm_listing(code_asm_text, "control_speed_with_cpad__jumping")
    speed_jumping_branch_instr = instr__arm_b(speed_jumping_hook_addr, arm9_start_address + offset, True, True)
    ...
    ar_code += f"""
        ...
        427FFC3C 00000100
            5{movement_hook_addr:07X} {movement_orig_instr:08X}
                ...
                0{speed_hook_addr:07X} {speed_branch_instr:08X}
                0{speed_jumping_hook_addr:07X} {speed_jumping_branch_instr:08X}
        D2000000 00000000
    """
