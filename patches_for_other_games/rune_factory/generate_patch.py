...
RomInfo = namedtuple(
    'RomInfo',
    'arm7_ipc_send_msg, arm7_set_ipc_channel_handler, arm7_rtc_region_start, arm7_vblank_irq_end_address, arm7_rtc_init_function_call, arm9_code_position, controls_hook, fix_aiming_hook, vec_normalize_address, description')
roms_info = {
    "ARFP-24CE355E": RomInfo(0x037FEAEC, 0x037FEBB8, 0x027F694C, 0x037FBB94, 0x037F84E0, 0x02000A78, 0x02034CB8, 0x02021924, 0x020BBB50, "Europe v1.0"),
    "ARFJ-588F9DF0": RomInfo(0x037FE9EC, 0x037FEAB8, 0x027F6798, 0x037FBAB4, 0x037F84E0, 0x02000A6C, 0x02034574, 0x0202164C, 0x020BB1B4, "Japan v1.0"),
    # "ARFK-2AB11BB4": RomInfo([], [], [], [], [], 0x02000AB0, [], [], [], "Korea v1.0"), # completely different memory offsets, newer SDK?
    "ARFE-57C35D6D": RomInfo(0x037FEAEC, 0x037FEBB8, 0x027F694C, 0x037FBB94, 0x037F84E0, 0x02000A78, 0x02034B38, 0x02021924, 0x020BB0F8, "USA v1.0"),
}
...
def assemble_arm9_patch(rom_signature, asm_filename):
    ...
    arm9_defines_for_assembler = [
        ("VEC_NORMALIZE_FUNC", rom_info.vec_normalize_address),
    ]
...
def generate_action_replay_code(rom_signature):
    ...
    # Player Movement
    player_move_hook_addr = rom_info.controls_hook
    player_move_orig_instr = 0xe5851020
    player_move_offset = find_function_offset_in_asm_listing(code_asm_text, "move_with_cpad")
    player_move_branch_instr = instr__arm_b(player_move_hook_addr, arm9_start_address + player_move_offset, True)

    # Fix aiming square position
    fix_aiming_square_hook_addr = rom_info.fix_aiming_hook
    fix_aiming_square_orig_instr = 0xe5912010
    fix_aiming_square_offset = find_function_offset_in_asm_listing(code_asm_text, "fix_aiming_square")
    fix_aiming_square_branch_instr = instr__arm_b(fix_aiming_square_hook_addr, arm9_start_address + fix_aiming_square_offset, True)
    ...
    ar_code += f"""
        ...
        # Fix aiming square position (round angle to nearest 45 degrees, as if we were using DPad)
        427FFC3C 00000100
            5{fix_aiming_square_hook_addr:07X} {fix_aiming_square_orig_instr:08X}
                0{fix_aiming_square_hook_addr:07X} {fix_aiming_square_branch_instr:08X}
        D2000000 00000000
    """
