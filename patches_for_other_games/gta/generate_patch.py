...
RomInfo = namedtuple(
    'RomInfo',
    'arm7_ipc_send_msg, arm7_set_ipc_channel_handler, arm7_rtc_region_start, arm7_vblank_irq_end_address, arm7_rtc_init_function_call, arm9_controls_hook_insertion_point, arm9_getangle_func, arm9_setup_mpu, description')
roms_info = {
    "YGXP-40BF4673": RomInfo(0x037FE39C, 0x037FE324, 0x027F5D48, 0x037FB718, 0x037F82D8, 0x02003716, 0x021332D0, 0x02000A78, "Europe v1.0"),
    "YGXJ-2D3EC547": RomInfo(0x037FE39C, 0x037FE324, 0x027F5D48, 0x037FB718, 0x037F82D8, 0x020041EA, 0x02133E34, 0x02000A78, "Japan v1.0"),
    "YGXE-DEE4AA12": RomInfo(0x037FE39C, 0x037FE324, 0x027F5D48, 0x037FB718, 0x037F82D8, 0x02003716, 0x021332D0, 0x02000A78, "USA v1.0"),
}
...
def assemble_arm9_patch(rom_signature, asm_filename):
    ...
    arm9_defines_for_assembler = [("GET_ANGLE_FUNC", rom_info.arm9_getangle_func)]
...
def generate_action_replay_code(rom_signature):
    ...
    ####################################################################################
    # Arm9 patch
    arm9_start_address = rom_info.arm9_setup_mpu
    code_binary, code_asm_text = assemble_arm9_patch(rom_signature, "arm9_controls_hook.s")

    # Player Movement
    hook_addr = rom_info.arm9_controls_hook_insertion_point
    orig_instr = 0x1100
    branch_instr = instr__thumb_bl(hook_addr+2, arm9_start_address, True)

    ar_code += f"""
        # wait for some time, just to be sure (0x27FFC3C is a frame counter)
        427FFC3C 00000600
            # check if we can hook the movement (9 - 16bit "if equal")
            9{hook_addr:07X} {orig_instr:08X}
                {ar_code__bulk_write(code_binary, arm9_start_address)} # main patch

                1{hook_addr:07X} {0xb500:08X}   # push "lr"
