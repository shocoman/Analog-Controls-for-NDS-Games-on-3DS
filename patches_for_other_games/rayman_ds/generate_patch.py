...
ARM9_CODE_START_ADDRESS = 0x02000a14  # function that setups Arm9 CPU's memory regions at the start
# ARM9_CODE_START_ADDRESS = 0x023BF000
...
def assemble_arm7_rtcom_patch(rom_signature):
    ...
    try:
        subprocess.check_output([make_exe_path, '--directory', arm7_patch_dir],
...
def assemble_arm9_patch(rom_signature, asm_filename):
    ...
    subprocess.check_output([asm_exe_path, asm_filename, '-o', f'{tmp_folder_name}/{patch_name}.out'])
...
def generate_action_replay_code(rom_signature):
    ...
    branch_to_rtcom_update_instruction = instr__arm_b(rom_info.arm7_vblank_irq_end_address,
                                                      rom_info.arm7_rtc_region_start + update_rtcom_func_offset)

    ar_code += f"""
        # wait until the arm7's code has been fully uploaded
        5{rom_info.arm7_vblank_irq_end_address:07X} {0xe12fff1e:08X}                   # if equal to "bx lr"

            0{rom_info.arm7_rtc_init_call_addr:07X} 00000000                           # prevent call to init the RTC on Arm7
            {ar_code__bulk_write(arm7_patch_bytes, rom_info.arm7_rtc_region_start)}    # write the Arm7 + Arm11 code

            0{rom_info.arm7_vblank_irq_end_address:07X} {branch_to_rtcom_update_instruction:08X}  # Hook the VBlank IRQ Handler
        D2000000 00000000
    """

    ####################################################################################
    # Arm9 Controls Patch
    branch_from_addr = rom_info.arm9_controls_hook_addr
    orig_instruction = 0xe3500000
    code_binary = assemble_arm9_patch(rom_signature, "arm9_controls_hook.s")
    new_branch_instr = instr__arm_b(branch_from_addr, ARM9_CODE_START_ADDRESS, True)

    ar_code += f"""
        9{branch_from_addr:07X} {orig_instruction:08X}
            {ar_code__bulk_write(code_binary, ARM9_CODE_START_ADDRESS)}
            0{branch_from_addr:07X} {new_branch_instr:08X}
        D2000000 00000000
    """
