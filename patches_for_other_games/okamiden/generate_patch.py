...
RomInfo = namedtuple(
    'RomInfo',
    'arm7_ipc_send_msg, arm7_set_ipc_channel_handler, arm7_rtc_region_start, arm7_vblank_irq_end_address, arm7_rtc_init_function_call, arm9_code_position, controls_for_fighting_hook, controls_for_freeroam_hook, controls_for_unknown_mode_hook, sincos_lookup_table, getangle_func_address, description')
roms_info = {
    "BOOP-FCF7B95E": RomInfo(0x037FEBEC, 0x037FEB7C, 0x027E0000, 0x037F8464, 0x037F8324, 0x02000A78, 0x0208A8C2, 0x0208A720, 0x0208AEF8, 0x02069780, 0x02002FDC, "Europe v1.0"),
    "BOOJ-36CAB1F1": RomInfo(0x037FEBEC, 0x037FEB7C, 0x027E0000, 0x037F8464, 0x037F8324, 0x02000A78, 0x0208B002, 0x0208AE60, 0x0208B638, 0x02069EAC, 0x02002FDC, "Japan v1.0"),
    "BOOE-8B046938": RomInfo(0x037FEBEC, 0x037FEB7C, 0x027E0000, 0x037F8464, 0x037F8324, 0x02000A78, 0x0208A8C2, 0x0208A720, 0x0208AEF8, 0x02069780, 0x02002FDC, "USA v1.0"),
}
...
def assemble_arm9_patch(rom_signature, asm_filename):
    ...
    arm9_defines_for_assembler = [("SINCOS_LOOKUP_TABLE", rom_info.sincos_lookup_table),
...
def generate_action_replay_code(rom_signature):
    ...
    ####################################################################################
    # Arm9 patch
    arm9_code_address = rom_info.arm9_code_position
    code_binary, code_asm_text = assemble_arm9_patch(rom_signature, "arm9_controls_hook.s")

    freeroam_and_fighting_offset = find_function_offset_in_asm_listing(code_asm_text, "FightingAndFreeroamMode_Entry")
    unknown_mode_offset = find_function_offset_in_asm_listing(code_asm_text, "UnknownPlayMode_Entry")

    # Controls for  Movement
    freeroam_hook_addr = rom_info.controls_for_freeroam_hook
    fighting_hook_addr = rom_info.controls_for_fighting_hook
    freeroam_and_fighting_orig_instr = 0xa806  # add r0,sp,#0x18
    freeroam_branch_instr = instr__thumb_bl(freeroam_hook_addr, arm9_code_address + freeroam_and_fighting_offset, True)
    fighting_branch_instr = instr__thumb_bl(fighting_hook_addr, arm9_code_address + freeroam_and_fighting_offset, True)

    unknown_mode_hook_addr = rom_info.controls_for_unknown_mode_hook
    unknown_mode_orig_instr = 0xa803  # add r0,sp,#0xc
    unknown_mode_branch_instr = instr__thumb_bl(fighting_hook_addr, arm9_code_address + unknown_mode_offset, True)
