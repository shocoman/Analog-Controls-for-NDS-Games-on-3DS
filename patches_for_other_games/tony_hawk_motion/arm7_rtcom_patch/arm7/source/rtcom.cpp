...
#include "../../arm11_ucode/source/rtcom_commands.h"
...
__attribute__((section(".text"))) static int COUNTER = 0;
__attribute__((section(".text"))) static struct {
    s32 index = 0;
    s32 xs[8] = { 0, 0, 0, 0, 0, 0, 0, 0 };
    s32 ys[8] = { 0, 0, 0, 0, 0, 0, 0, 0 };
} accel_history;
...
static void Update_CPad_etc() {
    ...
    // Read accelerometer [Y1, X2, X1], [Z2, Z1, Y2]
    readCmd = RTC_READ_ALARM_DATE_1_EXT;
    ...
    s16 accel_x = (accel[1] << 8) | accel[0];
    s16 accel_y = (accel[3] << 8) | accel[2];
    s16 accel_z = (accel[5] << 8) | accel[4];

    if ((REG_KEYINPUT & KEY_SELECT) == 0) { // dont smooth out the accelerometer if SELECT is pressed
        *(vu16 *)(RTCOM_DATA_OUTPUT + 10) = accel_x;
        *(vu16 *)(RTCOM_DATA_OUTPUT + 12) = accel_y;
        *(vu16 *)(RTCOM_DATA_OUTPUT + 14) = accel_z;
    } else {
        accel_history.xs[accel_history.index] = accel_x;
        accel_history.ys[accel_history.index] = accel_y;
        accel_history.index = (accel_history.index + 1) % 8;

        #define AVG(arr) ((arr[0] + arr[1] + arr[2] + arr[3] + arr[4] + arr[5] + arr[6] + arr[7] + 4) / 8)
        *(vu16 *)(RTCOM_DATA_OUTPUT + 10) = AVG(accel_history.xs);
        *(vu16 *)(RTCOM_DATA_OUTPUT + 12) = AVG(accel_history.ys);
        *(vu16 *)(RTCOM_DATA_OUTPUT + 14) = accel_z;
        #undef AVG
        *(vu16 *)(RTCOM_DATA_OUTPUT + 6) = accel_x;
        *(vu16 *)(RTCOM_DATA_OUTPUT + 8) = accel_y;
    }
}
...
__attribute__((target("arm"))) void RtcChannelHandler(int n_service, int arg, int no_handle) {
    ...
    rtcTransferReversed(&readCmd, 1, date_time_addr, 7, true);
    for (int i = 0; i < 7; i++) {
...
__attribute__((target("arm"))) void Update_RTCom() {
    ...
    case Start:
        ...
        if (*(u32*)ARM7_IPC_CHANNEL_HANDLER == 0xe92d4038)
