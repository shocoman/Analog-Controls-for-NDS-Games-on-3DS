...
RomInfo = namedtuple(
    'RomInfo',
    'arm7_ipc_send_msg, arm7_set_ipc_channel_handler, arm7_rtc_region_start, arm7_vblank_irq_end_address, arm7_rtc_init_function_call, arm9_code_position, arm9_is_motion_pak_present_check, arm9_is_motion_pak_absent_check, arm9_tony_hawk_motion_pak_check, arm9_tony_hawk_read_accel, arm9_hue_pixel_painter_read_accel, description')
roms_info = {
    "CTWP-51C5963F": RomInfo(0x037FEA2C, 0x037FE9B8, 0x027F5FA8, 0x037F850C, 0x037F8438, 0x02000AC8, 0x02069EC8, 0x02002490, 0x0202A824, 0x02039BF8, 0x02069E4A, "Europe v1.0"),
    "CTWE-88B86E61": RomInfo(0x037FEA2C, 0x037FE9B8, 0x027F5FA8, 0x037F850C, 0x037F8438, 0x02000AC8, 0x02069EC8, 0x02002490, 0x0202A824, 0x02039BF8, 0x02069E4A, "USA v1.0"),
}
...
# cache to speed up the building of multiple versions
arm7_build_cache: dict[tuple[int, ...], tuple[int, bytes]] = dict()
def assemble_arm7_rtcom_patch(rom_signature):
    ...
    try:
        subprocess.check_output([ld_exe_path, 'rtcom.o', f'{arm7_patch_dir}.uc11.o', '--output', 'arm7_patch.o',
                                 '--section-start', f'.text={rom_info.arm7_rtc_region_start:X}'],
                                stderr=subprocess.STDOUT, cwd=f'{arm7_patch_dir}/arm7/build/')
    except subprocess.CalledProcessError as e:
        print(e.output.decode('utf-8'))
        exit(-1)
...
def assemble_arm9_patch(rom_signature, asm_filename):
    ...
    arm9_defines_for_assembler = [
        # ("TOGGLE_VISOR_FUNC", rom_info.toggle_visor_func)
    ]
...
def generate_action_replay_code(rom_signature):
    ...
    is_motion_pack_present_check_addr = rom_info.arm9_is_motion_pak_present_check
    is_motion_pack_absent_check_addr = rom_info.arm9_is_motion_pak_absent_check
    tony_hawk_motion_check_addr = rom_info.arm9_tony_hawk_motion_pak_check

    tony_read_accel_hook_addr = rom_info.arm9_tony_hawk_read_accel
    tony_read_accel_orig_instr = 0xe1800c01
    arm9_offset = find_function_offset_in_asm_listing(code_asm_text, 'tony_hawk_read_accelerometer')
    tony_read_accel_branch_instr = instr__arm_b(tony_read_accel_hook_addr, arm9_start_address + arm9_offset, True)

    hue_read_accel_hook_addr = rom_info.arm9_hue_pixel_painter_read_accel
    hue_read_accel_orig_instr = 0xf7ff
    arm9_offset = find_function_offset_in_asm_listing(code_asm_text, 'hue_pixel_painter_read_accelerometer')
    hue_read_accel_branch_instr = instr__thumb_bl(hue_read_accel_hook_addr, arm9_start_address + arm9_offset, True)
    

    ar_code += f"""
        # load into a specific scene at launch
        # 527ffc20 00000000
        #     027ffc20 00000002 # skateboard
        #     # 027ffc20 000a0102 # snowboard
        #     # 027ffc20 80000001 # just skip intros
        # D2000000 00000000

        # patch the check isMotionPackPresent
        9{is_motion_pack_present_check_addr:07X} 000060C1
            1{is_motion_pack_present_check_addr:07X} 00000000
        D2000000 00000000
        
        # patch the check isMotionPackAbsent
        9{is_motion_pack_absent_check_addr:07X} 0000BD08
            1{is_motion_pack_absent_check_addr:07X} 00000000
        D2000000 00000000
        
        # patch the initial motion pack check in Tony Hawk
        5{tony_read_accel_hook_addr-0x50:07X} 3afffff3
            0{tony_read_accel_hook_addr-0x50:07X} 00000000
        D2000000 00000000
        
        # patch out another check for the motion pack in Tony Hawk
        9{tony_hawk_motion_check_addr:07X} 0000D105
            1{tony_hawk_motion_check_addr + 0x10:07X} 00000000
            1{tony_hawk_motion_check_addr + 0x12:07X} 00000000
            1{tony_hawk_motion_check_addr + 0x1E:07X} 00000000
            1{tony_hawk_motion_check_addr + 0x20:07X} 00000000
        D2000000 00000000
        ...
        427FFC3C 00000028
            6{arm9_start_address:07X} {int.from_bytes(code_binary[:4], 'little'):08X}
                {ar_code__bulk_write(code_binary, arm9_start_address)} # main patch
        D2000000 00000000
        
        # apply accelerometer in tony hawk
        427FFC3C 00000028
            5{tony_read_accel_hook_addr:07X} {tony_read_accel_orig_instr:08X}
                0{tony_read_accel_hook_addr:07X} {tony_read_accel_branch_instr:08X}
                0{tony_read_accel_hook_addr-0x48:07X} 00000000
        D2000000 00000000
        
        # apply accelerometer in hue pixel painter
        427FFC3C 00000028
            9{hue_read_accel_hook_addr:07X} {hue_read_accel_orig_instr:08X}
                1{hue_read_accel_hook_addr+0:07X} {hue_read_accel_branch_instr[0]:08X}
                1{hue_read_accel_hook_addr+2:07X} {hue_read_accel_branch_instr[1]:08X}
        D2000000 00000000
