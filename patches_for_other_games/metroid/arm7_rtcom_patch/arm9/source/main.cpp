...
int main() {
    ...
    while (true) {
        ...
        u8 zlzr = *(vu8 *)(RTCOM_DATA_OUTPUT + 4);
        s16 nub_x = *(vs16 *)(RTCOM_DATA_OUTPUT + 6);
        s16 nub_y = *(vs16 *)(RTCOM_DATA_OUTPUT + 8);
        iprintf("\x1b[%d;0H\t ZL&ZR: %08X", 4, zlzr);
        iprintf("\x1b[%d;0H \x1b[2K \tNub: {X: %+05d; Y: %+05d} \n ", 5, nub_x, nub_y);


        vu16 *gyro = (vu16 *)(RTCOM_DATA_OUTPUT + 10);
        s16 gyro_x = *gyro;
        s16 gyro_y = *(gyro + 1);
        s16 gyro_z = *(gyro + 2);
        iprintf("\x1b[%d;0H\x1b[2K Gyro: %+07d; %+07d; %+07d\n", 8, gyro_x, gyro_y, gyro_z);
        iprintf("\x1b[%d;0H\x1b[2K RawGyro: %04x; %04x; %04x\n", 9, (u16)gyro_x, (u16)gyro_y, (u16)gyro_z);
        ...
        iprintf("\x1b[%d;0H\x1b[2K Date: %02x; %02x; %02x; %02x\n", 11, reverseBits(year),
                reverseBits(month), reverseBits(day), reverseBits(dow));
        iprintf("\x1b[%d;0H\x1b[2K Time: %02x; %02x; %02x\n", 12, reverseBits(hour),
                reverseBits(minute), reverseBits(second));
