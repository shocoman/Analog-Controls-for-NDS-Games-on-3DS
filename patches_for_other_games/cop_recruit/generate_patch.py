...
RomInfo = namedtuple(
    'RomInfo',
    'arm7_ipc_send_msg, arm7_set_ipc_channel_handler, arm7_rtc_region_start, arm7_vblank_irq_end_address, arm7_rtc_init_function_call, arm9_code_position, dpad_hook, getangle_func_address, arm9_div_function, description')
roms_info = {
    "BDUP-F369D0AD": RomInfo(0x037FE410, 0x037FE39C, 0x027F5CC4, 0x037F84D8, 0x037F8414, 0x02000D00, 0x02053110, 0x0221F41C, 0x03806C38, "Europe v1.0"),
    "BDUE-E1E6F9A5": RomInfo(0x037FE410, 0x037FE39C, 0x027F5CC4, 0x037F84D8, 0x037F8414, 0x02000D00, 0x02053110, 0x0221F41C, 0x03806C38, "USA v1.0"),
}
...
def assemble_arm9_patch(rom_signature, asm_filename):
    ...
    arm9_defines_for_assembler = [
        ("GET_ANGLE_FUNC", rom_info.getangle_func_address),
    ]
...
def generate_action_replay_code(rom_signature, include_cstick_support):
    ...
    # Movement
    move_x_hook_addr = 0x02052f90
    move_x_orig_instr = 0xe5c34000
    arm9_offset = find_function_offset_in_asm_listing(code_asm_text, 'MoveWithCpad_X')
    move_x_branch_instr = instr__arm_b(move_x_hook_addr, arm9_start_address + arm9_offset, True)

    move_y_hook_addr = 0x02053034
    arm9_offset = find_function_offset_in_asm_listing(code_asm_text, 'MoveWithCpad_Y')
    move_y_branch_instr = instr__arm_b(move_y_hook_addr, arm9_start_address + arm9_offset, True)

    move_setdir_hook_addr = 0x02053110
    arm9_offset = find_function_offset_in_asm_listing(code_asm_text, 'MoveWithCpad_SetDudeDirection')
    move_setdir_branch_instr = instr__arm_b(move_setdir_hook_addr, arm9_start_address + arm9_offset, True)

    # Camera turning left/right
    cam_turn_1_hook_addr = 0x02052e38
    arm9_offset = find_function_offset_in_asm_listing(code_asm_text, 'TurnCameraWithCStick_Part_1')
    cam_turn_1_branch_instr = instr__arm_b(cam_turn_1_hook_addr, arm9_start_address + arm9_offset, True)

    cam_turn_2_hook_addr = 0x02052e7c
    arm9_offset = find_function_offset_in_asm_listing(code_asm_text, 'TurnCameraWithCStick_Part_2')
    cam_turn_2_branch_instr = instr__arm_b(cam_turn_2_hook_addr, arm9_start_address + arm9_offset, True)

    # Movement and turning the camera during the Aiming phase
    move_while_aiming_hook_addr = 0x02052c58
    arm9_offset = find_function_offset_in_asm_listing(code_asm_text, 'WalkDuringAimingWithCPad')
    move_while_aiming_branch_instr = instr__arm_b(move_while_aiming_hook_addr, arm9_start_address + arm9_offset, True)
    
    aiming_with_cstick_hook_addr = 0x0205283c
    arm9_offset = find_function_offset_in_asm_listing(code_asm_text, 'AimingWithCStick')
    aiming_with_cstick_x_branch_instr = instr__arm_b(aiming_with_cstick_hook_addr, arm9_start_address + arm9_offset, True)

    # Car Steering left/right
    car_steer_right_hook_addr = 0x0222deec
    arm9_offset = find_function_offset_in_asm_listing(code_asm_text, 'CarSteering')
    car_steer_right_branch_instr = instr__arm_b(car_steer_right_hook_addr, arm9_start_address + arm9_offset, True)

    car_steer_left_hook_addr = 0x0222dea4
    car_steer_left_branch_instr = instr__arm_b(car_steer_left_hook_addr, arm9_start_address + arm9_offset, True)

    ar_code += f"""
        # wait for some time, just to be sure (0x27FFC3C is a frame counter)
        427FFC3C 00000200
            5{move_x_hook_addr:07X} {move_x_orig_instr:08X}
                {ar_code__bulk_write(code_binary, arm9_start_address)} # main patch
                0{move_x_hook_addr:07X} {move_x_branch_instr:08X}
                0{move_y_hook_addr:07X} {move_y_branch_instr:08X}
                0{move_setdir_hook_addr:07X} {move_setdir_branch_instr:08X}

                # prevent the main character from stopping while the camera is turning 
                02052e48 00000000 # nop move_x = 0
                02052e50 00000000 # nop move_y = 0
                02052e58 00000000 # nop walk_state = 0
                02052f00 00000000 # don't skip the usual movement routine

                # camera turning left or right
                0{cam_turn_1_hook_addr:07X} {cam_turn_1_branch_instr:08X}
                0{cam_turn_2_hook_addr:07X} {cam_turn_2_branch_instr:08X}

                # aiming
                0{move_while_aiming_hook_addr:07X} {move_while_aiming_branch_instr:08X}
                0{aiming_with_cstick_hook_addr:07X} {aiming_with_cstick_x_branch_instr:08X}

                # steering
                0{car_steer_left_hook_addr:07X} {car_steer_left_branch_instr:08X}
                0{car_steer_right_hook_addr:07X} {car_steer_right_branch_instr:08X}
