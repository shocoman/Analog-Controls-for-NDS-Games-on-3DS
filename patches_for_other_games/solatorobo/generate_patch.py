...
RomInfo = namedtuple(
    'RomInfo',
    'arm7_ipc_send_msg, arm7_set_ipc_channel_handler, arm7_rtc_region_start, arm7_vblank_irq_end_address, arm7_rtc_init_function_call, arm9_code_position, arm9_controls_hook, arm9_veclength, arm9_div32, getangle_func_address, description')
roms_info = {
    "VCDV-471E19DD": RomInfo(0x037FFB64, 0x037FFAF4, 0x02FE0000, 0x037FC020, 0x037F80CC, 0x02004A98, 0x021982F8, 0x02013890, 0x0200BAE8, 0x02013D14, "Europe v1.0"),
    "VCDJ-7A2EF8AD": RomInfo(0x037FFB64, 0x037FFAF4, 0x02FE0000, 0x037FC020, 0x037F80CC, 0x02004A98, 0x021A4AB4, 0x02013890, 0x0200BAE8, 0x02013D14, "Japan v1.0"),
    "VCDE-3C1E9F64": RomInfo(0x037FFB64, 0x037FFAF4, 0x02FE0000, 0x037FC020, 0x037F80CC, 0x02004A98, 0x0218DDB8, 0x02012D18, 0x0200B184, 0x0201319C, "USA v1.0"),
}
...
def assemble_arm9_patch(rom_signature, asm_filename):
    ...
    arm9_defines_for_assembler = [
        ...
        ("VEC_LENGTH_FUNC", rom_info.arm9_veclength),
        ("DIV32_FUNC", rom_info.arm9_div32),
    ]
...
def generate_action_replay_code(rom_signature):
    ...
    # Player Movement
    hook_addr = rom_info.arm9_controls_hook
    orig_instr = 0xe0852000
    branch_instr = instr__arm_b(hook_addr, arm9_start_address, True)
